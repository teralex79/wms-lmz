...
proc Meas_SWMS_old {name join n} {
global wms
  ...
  catch {global x${name}IIo y${name}IIo}
  catch {x${name}IIo  set ""}
  catch {y${name}IIo  set ""}
  catch {x${name}Spec set ""}
  catch {y${name}Spec set ""}

  for {set i 15} {$i<1029} {incr i} {
    set lmd($i) [lindex $wms($name,swms,lamda) $i]
    set Icalc($i) [lindex $wms($name,swms,Icalc,$n,$join) $i]
    if {$n!="k"} {
      if {!$join} {
        set Io($i) $wms($name,$lmd($i),Io)
        if {$Io($i)==0} {set Io($i) 1}
        set x${name}IIo(++end) $lmd($i)
        set y${name}IIo(++end) [format "%4.2f" [expr {1.0*$Icalc($i)/$Io($i)}]]
      }
    }
    set x${name}Spec(++end) $lmd($i)
    set y${name}Spec(++end) $Icalc($i)
  }
  set wms(ready) 1
  incr wms($name,cont)
}
